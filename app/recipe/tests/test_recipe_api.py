...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_delete_recipe(self):
        """Test deleteing a recipe seuccessful."""
        recipe = create_recipe(user=self.user)
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        tag_idian = Tag.objects.create(user=self.user, name='Indian')
        payload = {
            'title': 'Pongal',
            'time_minutes': 60,
            'price': Decimal('4.50'),
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.tags.count(), 2)
        self.assertIn(tag_idian, recipe.tags.all())
        for tag in payload['tags']:
            exists = recipe.tags.filter(
                name=tag['name'],
                user=self.user,
    ...
    def create_tag_on_update(self):
        ...
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='jason')
    ...
    def test_update_recipe_assign_tag(self):
        """Test assigning as existing tag when updating a recipe."""
